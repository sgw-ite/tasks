...
class DTClassifier():
    ...
    def __init__(self, max_gini=0.2, min_samples=6):  
        self.Tree = None
    ...
    def _CreateTree(self, X, y):
        ...
        if ((self._gini(y_l) <= self.max_gini) or (len(y_l)<=self.min_samples)):
            if ((self._gini(y_r) <= self.max_gini) or (len(y_r)<=self.min_samples)):
                Tree = {(d,value,0):np.median(y_l), (d,value,1):np.median(y_r)}
            else:
                Tree = {(d,value,0):np.median(y_l), (d,value,1):self._CreateTree(X_r, y_r)}
        else:
            if ((self._gini(y_r) <= self.max_gini) or (len(y_r)<=self.min_samples)):
                Tree = {(d,value,0):self._CreateTree(X_l, y_l), (d,value,1):np.median(y_r)}
            else:
